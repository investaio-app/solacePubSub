...
var GuaranteedRequestor = function (requestQName) {
    'use strict';
    var requestor = {};
    requestor.session = null;
    requestor.requestQName = requestQName;
    ...
    requestor.log('\n*** requestor to topic "' + requestor.requestQName + '" is ready to connect ***');
    ...
    requestor.request = function () {
        if (requestor.session !== null) {
            ...
            const replyMessageConsumer = requestor.session.createMessageConsumer({
                queueDescriptor: { name: 'Reply', type: solace.QueueType.QUEUE, durable: true },
            });
            ...
            replyMessageConsumer.on(solace.MessageConsumerEventName.UP, function () {
                replyMessageConsumer.active = true;
                var msg = solace.SolclientFactory.createMessage();
                const requestText = "This is my request";
                requestor.log('Sending request "' + requestText + '" to request topic "' + requestor.requestQName + '"...');
                // msg.setDestination(solace.SolclientFactory.createTopicDestination(requestor.requestTopicName));
                msg.setDestination(solace.SolclientFactory.createDurableQueueDestination(requestor.requestQName))
                msg.setBinaryAttachment(requestText);
                msg.setReplyTo(solace.SolclientFactory.createDurableQueueDestination('Reply'));
            ...
            replyMessageConsumer.on(solace.MessageConsumerEventName.MESSAGE, function onMessage(message) {
                ...
                // replyMessageConsumer.disconnect();
            });

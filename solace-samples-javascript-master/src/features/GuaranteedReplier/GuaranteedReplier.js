...
var GuaranteedReplier = function (queueName) {
    'use strict';
    var replier = {};
    replier.session = null;
    replier.messageConsumer = null;
    replier.queueName = queueName;
    ...
    replier.log('\n*** replier to request topic "' + replier.queueName + '" is ready to connect ***');
            ...
            if (replier.active) {
                ...
                try {
                    replier.messageConsumer = replier.session.createMessageConsumer({
                        ...
                        queueDescriptor: { name: replier.queueName, type: solace.QueueType.QUEUE }
                    ...
                    replier.messageConsumer.on(solace.MessageConsumerEventName.UP, function () {
                        ...
                        replier.log('Replier is consuming from temporary topic endpoint,' +
                            ' which is attracting messages to "' + replier.queueName + '"');
                    });
    ...
    replier.reply = function (message) {
        ...
        if (replier.session !== null) {
            try {
                ...
                replyMsg.setDestination(solace.SolclientFactory.createDurableQueueDestination('Reply'));
    ...
    replier.stopService = function () {
        if (replier.session !== null) {
            if (replier.active) {
                replier.active = false;
                replier.log('Disconnecting from request topic endpoint: ' + replier.queueName);
                try {
